...
function perf_test()
    ...
    CSV.write("scatter_perf_$(Threads.nthreads())_nxcell_fma_3D.csv", df)
    
end

perf_test()

# files = readdir(".")[endswith.(readdir("."), "fma_3D.csv")]
...
# ## TIMINGS
# f = Figure(fontsize=20, resolution=(1200, 1200))

# ax1 = Axis(f[1,1], title="scatter (grid → particle)", ylabel="seconds", xscale = log10, yscale = log10)
# ax2 = Axis(f[2,1], title="gather (particle → grid)", xlabel="particles", ylabel="seconds", xscale = log10, yscale = log10)
# for data in out
