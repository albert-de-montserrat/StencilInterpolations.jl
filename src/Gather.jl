function _gather(p::NTuple{N, A}, dxi::NTuple{N, B}, xi::NTuple{N, C}, F::Array{D, N}) where {A,B,C,D,N}
...
function gather(xi, dxi, F::Array{T, N}, particle_coords) where {T, N}
    
    np = length(particle_coords[1])
    Fp = zeros(T, np)

    Threads.@threads for i in 1:np
        @inbounds Fp[i] = _gather((particle_coords[1][i], particle_coords[2][i]), dxi, xi, F)
    end

    Fp
end

function foo!(Fp::CuDeviceVector{T, 1}, p::NTuple{N, A}, dxi::NTuple{N, B}, xi::NTuple{N, C}, F::CuDeviceMatrix{T, 1}) where {A,B,C,N, T}
